...
def _crc8(pc_block: bytearray) -> int:
    crc = 0xFF
    size = len(pc_block)
    print(size)
    for i in range(size):
        crc = _Crc8Table[crc ^ pc_block[i]]
    return crc
...
def decode_from_png_to_jpeg_with_crc8(png_filename: str, jpeg_filename: str) -> None:
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            print(y, x)
            print(color)
            ba = _crc8(bytearray(color.to_bytes(4, byteorder="big", signed=False)))
            print(ba)
            ba_rec.append(ba.to_bytes(1, byteorder="big", signed=False))
